...
public:
	...
	std::vector<std::pair<std::string, int>> get_all_pattern(std::string pattern) {
		static Finder<Index<StringSet<String<AminoAcid>>, FMIndex<> > > fmFinder(fmIndex);
		std::vector<std::pair<std::string, int>> res;

		while (find(fmFinder, pattern))
		{
			res.emplace_back(std::pair<std::string, int>(fmID_short[position(fmFinder).i1], position(fmFinder).i2));
		}
		clear(fmFinder);
		return res;
	}
